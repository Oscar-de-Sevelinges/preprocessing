...
string preprocessWord(string word, vector<string> stopList)
{
	std::vector<string>::iterator it = std::find(stopList.begin(), stopList.end(), word);
	if (it != stopList.end())
	{
		return "";
	}
	else
	{
		...
		{
			if ((word[i] >= 33 && word[i] <= 64) || (word[i] >= 91 && word[i] <= 96) || (word[i] >= 123 && word[i] <= 126))
			{
				std::string::iterator ite = word.begin() + i;
				word.std::string::erase(word.begin() + i);
				//word[i] = 0;
			}
...
// function that preprocess a file (conversion to lower case, deletion of special characters)
void preprocessFile(string fileName)
{
	// we open the stop list
	ifstream ifsStopList("stopList.txt");
	// we create a vector that contains each word of the stop list
	vector<string> stopList;
	string word;

	// while we can get a word from the stop list file
	while (!ifsStopList.eof())
	{
		ifsStopList >> word;
		// we add it in the vector
		stopList.push_back(word);
	}
	ifsStopList.close();

	// we open the file corresponding to the name we have in the parameters of the function
	ifstream ifs("./files/" + fileName + ".txt");
	if (!ifs)
	{
		runtime_error("fichier " + fileName + " introuvable");
	}

	// we store the whole file in a string
	vector<string> entireFile;

	// while we can get a word from the file
	while (!ifs.eof())
	{
		// we create a tmp string that contains the current word that we read in the file
		string tmp;
		ifs >> tmp;

		entireFile.push_back(preprocessWord(tmp, stopList));
	}
	ifs.close();

	// we create a txt file containing the preprocessed text
	ofstream ofs1("./preprocessedFiles/" + fileName + "-preprocessed.txt", std::ios::out | std::ios::trunc);
	ofstream ofs2("./preprocessedFiles/dataBase-preprocessed.txt", std::ios_base::app | std::ofstream::out);

	for (const auto& elem : entireFile)
	{
		ofs1 << elem + " ";
	}
	ofs2 << fileName + "-preprocessed\n";
    ...
    int i;
    s = (char*)malloc(i_max + 1);
    string path = "./ preprocessedFiles/" + fileName + "-preprocessed.txt";
...
int main()
{
	// vector containing the name of each document we need to preprocess
	vector<string> documents;
	// we open the file containing the name of each document
	ifstream ifs("./files/dataBase.txt");
	string docName;
	if (!ifs)
	{
	...
	// while we can get a name from the file
	while (!ifs.eof())
	{
	...
	std::filesystem::remove("./preprocessedFiles/dataBase-preprocessed.txt");

	// for each document of the vector
